...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    filter = referendum['Department code'].str.startswith('Z')
    referendum = referendum[~filter]

    rad = regions_and_departments
    rad['code_dep'] = rad['code_dep'].apply(
        lambda x: x[1:] if x.startswith('0') else x)

    df = pd.merge(rad, referendum,
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    res = referendum_and_areas.groupby(['name_reg']).sum()
    res = res.reset_index()[
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    df = pd.merge(regions, referendum_result_by_regions, on='name_reg')
    df['ratio'] = df['Choice A'] / (df['Choice A'] + df['Choice B'])
    df['percentage'] = df['ratio'] * 100
    df.plot(column='percentage', legend=True)
    return df
