...
def merge_regions_and_departments(regions, departments):
    ...
    df.drop(columns=["id_x", "slug_x", "id_y", "slug_y", "region_code"], inplace=True)
    df.rename(
        columns={
            "code_x": "code_reg",
            "name_x": "name_reg",
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    referendum_groupby = referendum_and_areas.groupby(["code_reg", "name_reg"]).sum()
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    regions = regions[["code", "nom", "geometry"]]
    ref_by_reg = pd.merge(
        regions,
    ...
    ref_by_reg["ratio"] = ref_by_reg["Choice A"] / (
        ref_by_reg["Choice B"] + ref_by_reg["Choice A"]
    )

    geo_df = gpd.GeoDataFrame(ref_by_reg)
    geo_df.plot("ratio")
