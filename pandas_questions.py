...
def merge_regions_and_departments(regions, departments):
    ...
    merged.rename(columns={'code_x': 'code_reg', 'name_x': 'name_reg',
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    """
    depart_code = referendum["Department code"].copy()
    referendum["Department code"] = depart_code.apply(lambda x: x.zfill(2))
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    aggregated = referendum_and_areas.groupby(
        ['name_reg', 'code_reg'], as_index=False).sum(numeric_only=True)
    aggregated.reset_index()
    aggregated.set_index("code_reg")

    return aggregated[['name_reg', 'Registered', 'Abstentions',
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    """
    geo_json = gpd.read_file("data/regions.geojson")
    referendum_result_by_regions['nom'] = \
        referendum_result_by_regions['name_reg']
    merged = gpd.GeoDataFrame(pd.merge(referendum_result_by_regions,
                                       geo_json, on='nom'))
