...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    df = pd.merge(referendum[~referendum['Department code'].str.startswith('Z')],
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    df = referendum_and_areas[columns].groupby(['code_reg', 'name_reg']).sum().reset_index('name_reg')
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    """
    gdf = gpd.read_file('data/regions.geojson').set_index('code')
    df = gdf.join(referendum_result_by_regions, how='right')
