...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    Data = pd.merge(ref, regions_and_departments,
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    Data2 = referendum_and_areas[['code_reg', 'name_reg',
                                  'Registered', 'Abstentions',
                                  'Null', 'Choice A', 'Choice B']]
    Data2[['Registered', 'Abstentions', 'Null', 'Choice A',
           'Choice B']] = Data2[['Registered', 'Abstentions', 'Null',
                                 'Choice A', 'Choice B']].apply(pd.to_numeric)
    Data = Data2.groupby('code_reg').agg({c: 'sum' if Data2[c].dtype == 'int64'
                                          else 'first' for c in Data2.columns})
    Data = Data[['name_reg', 'Registered', 'Abstentions', 'Null', 'Choice A',
                 'Choice B']]
    return Data
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    merged_data = pd.merge(referendum_result_by_regions,
