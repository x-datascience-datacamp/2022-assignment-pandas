...
def load_data():
    ...
    referendum = pd.read_csv(f"{dir}/referendum.csv", delimiter=';', on_bad_lines='skip')
    regions = pd.read_csv(f"{dir}/regions.csv", on_bad_lines='skip')
    departments = pd.read_csv(f"{dir}/departments.csv", delimiter=',', on_bad_lines='skip')
...
def merge_regions_and_departments(regions:pd.DataFrame, departments:pd.DataFrame):
    ...
    merged_reg_dep = pd.merge(regions[['code_reg', 'name_reg']], departments[['code_reg','code_dep', 'name_dep']], on='code_reg', how='left')
    
    
    return merged_reg_dep


def merge_referendum_and_areas(referendum:pd.DataFrame, regions_and_departments:pd.DataFrame):
    ...
    merged_ref_reg_dep = pd.merge(referendum, regions_and_departments, left_on='Department code', right_on='code_dep', how='inner')
...
def compute_referendum_result_by_regions(referendum_and_areas:pd.DataFrame):
    ...
    absolute_count = absolute_count[['name_reg', 'Registered', 'Abstentions', 'Null', 'Choice A', 'Choice B']]
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    ref_map = gpd.GeoDataFrame(pd.merge(referendum_result_by_regions, reg_json, on='nom'))
    ref_map.plot("ratio")
    return ref_map
