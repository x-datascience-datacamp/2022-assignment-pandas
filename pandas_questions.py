...
def merge_regions_and_departments(regions, departments):
    ...
    departments = departments\
        .rename(columns={'id': 'id_dep', 'region_code': 'code_reg',
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    name_regions = name_regions.groupby(['code_reg']).first()

    referendum_and_areas = \
        referendum_and_areas[['code_reg', 'Registered',
                              'Abstentions', 'Null', 'Choice A', 'Choice B']]
    referendum_and_areas = referendum_and_areas.groupby(['code_reg']).sum()

    return pd\
        .merge(name_regions, referendum_and_areas, on='code_reg', how='right')
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    referendum_result_by_regions = \
        pd.merge(map, referendum_result_by_regions, left_on='code',
                 right_on='code_reg')
    referendum_result_by_regions = \
        referendum_result_by_regions.drop(['code', 'nom'], axis=1)
