...
def merge_regions_and_departments(regions, departments):
    ...
    departments.rename(columns={"region_code": "code_reg", "code": "code_dep",
                       "name": "name_dep", "slug": "slug_dep"}, inplace=True)
    columns = ['code_reg', 'name_reg', 'code_dep', 'name_dep']
    return pd.merge(regions,
                    departments,
                    on="code_reg",
                    how="left")[columns]
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    res = referendum_and_areas[cols] \
        .groupby(by="name_reg").sum().reset_index()
    return res
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    """
    # Get ratio column
    all_expressed_ballots = referendum_result_by_regions['Choice A'] \
        + referendum_result_by_regions['Choice B']
    referendum_result_by_regions['ratio'] = \
        referendum_result_by_regions['Choice A'] \
        / all_expressed_ballots

    gdf = gpd.read_file('data/regions.geojson')

    res = pd.merge(gdf, referendum_result_by_regions,
