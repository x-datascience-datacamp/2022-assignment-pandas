...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    """
    return (referendum_and_areas.groupby(["code_reg", "name_reg"],
            as_index=False).agg(
             {"Registered": "sum", "Abstentions": "sum",
              "Null": "sum", "Choice A": "sum", "Choice B": "sum"}).set_index(
               "code_reg"))
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    geo_merge = pd.merge(geo_reg, referendum_result_by_regions,
                         left_on="code", right_index=True)
    geo_merge["ratio"] = (geo_merge["Choice A"] /
                          (geo_merge["Registered"]
                           - geo_merge["Abstentions"] - geo_merge["Null"]))
    geo_merge.plot(column="ratio", cmap="OrRd")
